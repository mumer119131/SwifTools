...
function buildIndex(tools: readonly Tool[]): Indexed[] {
  if (cache?.source === tools) return cache.index;
...
export function searchTools(query: string, tools: readonly Tool[]): SearchResult[] {
  ...
  for (const entry of index) {
    ...
    // Every token must match something. "png to pdf" should not return a tool
    // that only matches "to".
    ...
    scored.push({ tool: entry.tool, score: total, reason });
  }
